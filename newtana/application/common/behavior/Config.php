...
class Config
{
    ...
    public function run()
    {
        ...
        $pathArr = explode(config('pathinfo_depr'), $path);
        // 获取当前模块名称
        if (isset($pathArr[0])) {
            $module = $pathArr[0];
        }
        //检测路由
        // 是否强制路由模式
        $must = Route::config('url_route_must');
        // 路由检测 返回一个Dispatch对象
        $dispatch = Route::check($path, $must);
        //如果路由匹配
        if ($module != 'admin' && is_array($dispatch->getDispatch()) && !empty($dispatch->getDispatch()[0])){
            $module = $dispatch->getDispatch()[0];
        }
        // 获取入口目录
